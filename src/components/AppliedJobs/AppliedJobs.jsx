...
const AppliedJobs = () => {
  ...
  useEffect(() => {
    ...
    if (viewType == "all") {
      ...
      const fakeShownJobs = appliedJobs.filter(
        (aj) => aj.onsiteJob == "Remote"
      );
      setShownJobs(fakeShownJobs);
    } else if (viewType == "Onsite") {
      const fakeShownJobs = appliedJobs.filter(
        (aj) => aj.onsiteJob == "Onsite"
      );
      setShownJobs(fakeShownJobs);
    }
    ...
    <div>
      ...
      <div className="flex flex-col gap-3 details-container mt-5 md:mt-10 min-h-[45vh]">
        ...
        <div className="flex flex-col gap-3">
          {shownJobs.length > 0 ? (
            shownJobs.map((shownJob) => (
              <AppliedJob key={shownJob.id} shownJob={shownJob}></AppliedJob>
            ))
          ) : (
